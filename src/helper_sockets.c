...
int socket_object_init(SocketObject socket_obj, SocketType type, uint16_t sin_family, uint16_t sin_addr, uint16_t sin_port)
{
...
int socket_write(SocketObject socket_obj, char *string, uint16_t str_len)
{
    ...
    if(socket_obj.type == socket_type_server)
    {
        ...
        write(socket_obj.socket_client_fd, string, str_len);

    }
    else
    {

        write(socket_obj.socket_fd, string, str_len);
